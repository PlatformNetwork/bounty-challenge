...
pub async fn run(rpc: &str, hotkey: &str) -> Result<()> {
    ...
    if let Some(leaderboard) = result["leaderboard"].as_array() {
        let miner = leaderboard.iter().find(|e| {
            e["miner_hotkey"].as_str() == Some(hotkey)
        });
        ...
        if let Some(entry) = miner {
            ...
            let rank = leaderboard.iter().position(|e| {
                e["miner_hotkey"].as_str() == Some(hotkey)
            ...
            println!("Current Score:    {}", style_green(&format!("{:.4}", score)));
